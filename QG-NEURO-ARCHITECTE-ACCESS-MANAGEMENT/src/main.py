...
def generate_activation_key():
    """Génère une clé d'activation au format SUPRA-UNLOCK-XXXXXX"""
    # Génère 6 caractères alphanumériques aléatoires (lettres majuscules et chiffres)
    random_chars = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
    return f"SUPRA-UNLOCK-{random_chars}"
    ...
    if client_email not in db:
        db[client_email] = {
            "profil": {
                ...
                "accessLevel": "free",  # Par défaut, accès gratuit
                "activationKey": None   # Pas de clé d'activation par défaut
            },
            "resultats": []
        }
...
# --- 6. ROUTES ADMIN POUR LA GESTION DES ACCÈS ---

@app.route('/api/v1/operators/<email>/generate-key', methods=['POST'])
def generate_operator_key(email):
    """Génère une clé d'activation pour un opérateur"""
    ...
    # Sauvegarder la clé dans le profil de l'opérateur
    db[email]['profil']['activationKey'] = activation_key
    
    print(f"--- CLÉ D'ACTIVATION GÉNÉRÉE POUR {email}: {activation_key} ---")
...
@app.route('/api/v1/operators/<email>/grant-full-access', methods=['POST'])
def grant_full_access(email):
    """Accorde l'accès complet à un opérateur"""
    # Vérifier si l'opérateur existe
    if email not in db:
        return jsonify({"error": "Operator not found"}), 404
    
    # Changer le niveau d'accès à "full"
    db[email]['profil']['accessLevel'] = "full"
    
    print(f"--- ACCÈS COMPLET ACCORDÉ À {email} ---")
